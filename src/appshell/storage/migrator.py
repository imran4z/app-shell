...
from appshell.storage.db import connect
...
def _migration_files() -> list[Path]:
    ...
    pkg = files("appshell.storage.migrations")
    out: list[Path] = []
...
def drop_all(engine: Engine | None = None) -> None:
    """Drop every App Shell table (and the migrations log). Used by `db reset`.

    Does NOT drop the database itself or any non-App Shell tables. Order matters
    because of FK references; we use CASCADE to make it forgiving.
    """
    eng = engine or connect()
    # Drop in FK-safe order (children before parents). CASCADE catches
    # anything we miss but keeps explicit dependencies obvious in this list.
    #
    # CONTRACT: every new migration that creates a table MUST register it
    # here, child-first. Skipping this silently breaks `just db-reset`.
    tables = [
        # Cost ledger - no FKs.
        "llm_calls",
        # Assistant chat - turns FK into conversations; child first.
        "assistant_turns",
        "assistant_conversations",
        # The app's user directory.
        "users",
        # Example domain entities shipped with the template. Replace with
        # your app's tables as you add migrations.
        "profiles",
        "items",
        "_migrations",
    ]
